  ...
  return (
    <div className="fixed inset-0 bg-[#302782] dark:bg-gray-950 flex flex-col font-sans overflow-hidden">
      ...
      <header className="text-white pt-4 sm:pt-6 pb-4 px-6 flex flex-col items-center justify-center relative z-10 flex-shrink-0">
        ...
        <p className="text-white/70 text-xs sm:text-sm mt-1 text-center">สแกน QR หรือเลือกห้องจากรายการ</p>
      </header>
      ...
      {/* 🌟 Modal เลือกห้อง (ขยายขนาดเป็น max-w-xl ให้กว้างขึ้น) */}
      {showRoomModal && (
...
// ==========================================
// 📌 SUB-COMPONENTS (แยกออกมาให้ดูเป็นระเบียบ)
// ==========================================

// 1. Component แสดงรายการตึกและห้อง
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <p className="text-gray-500 font-medium text-lg">กำลังโหลดข้อมูลห้อง...</p>
      </div>
    );
  }

  if (groupedRooms.length === 0) {
    return <p className="text-center text-gray-500 py-20 text-lg">ไม่มีข้อมูลห้องในระบบ</p>;
  }
    ...
    <div className="space-y-8 pb-6">
      {groupedRooms.map(({ building, rooms: buildingRooms }) => (
        <div key={building} className="space-y-4">
          ...
          {/* 🏢 หัวข้อตึก (เพิ่มพื้นที่, ป้องกันข้อความ 5 ห้อง ตกบรรทัด) */}
          <div className="sticky top-0 bg-white/95 dark:bg-gray-800/95 backdrop-blur-md py-3 z-10 flex items-center gap-3 border-b-2 border-gray-100 dark:border-gray-700">
            <div className="bg-indigo-50 dark:bg-gray-700 p-2 rounded-xl text-[#302782] dark:text-[#B2BB1E] flex-shrink-0">
              <Building2 size={20} />
            </div>
            {/* ชื่ออาคาร ให้ยาวแค่ไหนก็ไม่ดันป้ายจำนวนห้อง */}
            <h3 className="font-bold text-gray-800 dark:text-white text-base sm:text-lg flex-1 pr-2">
              {building}
            </h3>
            {/* ป้ายจำนวนห้อง ใช้ whitespace-nowrap และ flex-shrink-0 เพื่อให้อยู่บรรทัดเดียวเสมอ */}
            <span className="text-sm font-bold text-[#302782] dark:text-[#B2BB1E] bg-indigo-100/50 dark:bg-gray-700 px-3 py-1.5 rounded-full whitespace-nowrap flex-shrink-0">
              {buildingRooms.length} ห้อง
            </span>
          </div>

          {/* รายการห้องในตึกนั้นๆ (ขยาย Padding ให้ปุ่มใหญ่ขึ้น กดง่ายขึ้น) */}
            ...
            {buildingRooms.map((room) => {
              ...
              return (
                <button
                  ...
                  disabled={!available}
                  className={`w-full text-left p-4 sm:p-5 rounded-2xl border-2 transition-all flex items-center justify-between group shadow-sm ${
                    available 
                ...
                >
                  <h4 className={`text-lg sm:text-xl font-extrabold ${
                    available 
                      ? "text-gray-800 dark:text-white group-hover:text-[#302782] dark:group-hover:text-[#B2BB1E]" 
                  ...
                  {available ? (
                    ...
                    <div className="flex items-center gap-1.5 text-red-500 dark:text-red-400 bg-red-50 dark:bg-red-900/30 px-3 py-1.5 rounded-xl">
                      <Lock size={14} />
...
const TabButton = ({ active, onClick, icon, label }) => (
  <Button variant="none" size="none" onClick={onClick} className={`flex-1 flex justify-center items-center py-3.5 rounded-xl text-xs font-bold gap-2 transition-all ${ active ? "bg-[#302782] shadow-md text-white scale-100" : "text-gray-500 dark:text-gray-400 hover:bg-white dark:hover:bg-gray-600 scale-95"}`}>
...
const GalleryUpload = ({ onFileChange, disabled }) => (
  <label className="flex flex-col items-center justify-center w-full h-full cursor-pointer bg-gray-50 dark:bg-gray-700 hover:bg-gray-100 transition-colors group">
    <div className="bg-[#B2BB1E]/10 group-hover:bg-[#B2BB1E]/20 p-8 rounded-full mb-4 transition-all transform group-active:scale-95">
      <ImageIcon size={50} className="text-[#302782] dark:text-white" />
    </div>
    <div className="text-center">
      <p className="text-[#302782] dark:text-white font-black text-base mb-1">เลือกรูปภาพจากคลัง</p>
      <p className="text-gray-400 text-xs text-center px-4">ระบบจะสแกน QR Code จากรูปที่คุณเลือก</p>
    </div>
