...
const RoomDetailView = ({ room, isFetchingQR, qrImageData, onSimulateScan, onBack }) => (
  <div className="flex flex-col items-center animate-in slide-in-from-right-4 duration-300 py-6">
    <div className="w-56 h-56 bg-white p-4 rounded-[32px] shadow-xl border border-gray-100 mb-8 flex items-center justify-center overflow-hidden relative">
      {isFetchingQR ? <div className="absolute inset-0 flex items-center justify-center bg-white/80 backdrop-blur-sm"><Loader2 size={40} className="animate-spin text-[#302782]" /></div> : qrImageData ? <img src={qrImageData} alt="QR Code" className="w-full h-full object-contain" /> : <p className="text-sm text-gray-400 font-medium">ไม่พบ QR Code</p>}
    </div>
    <h3 className="text-3xl font-black text-gray-800 dark:text-white text-center mb-2">ห้อง {room.room_number || room.room_id || room.name}</h3>
    <p className="text-lg text-gray-500 dark:text-gray-400 text-center mb-10 flex items-center justify-center gap-2"><Building2 size={20} />{room.location || room.building || "ไม่ระบุอาคาร"}</p>
    <div className="w-full max-w-sm space-y-3">
      <button onClick={() => onSimulateScan(room.id || room.room_id)} className="w-full bg-[#B2BB1E] text-[#302782] py-4 rounded-2xl font-black flex items-center justify-center gap-2 hover:bg-[#9ca31a] transition-all shadow-lg active:scale-95 text-lg"><QrCode size={24} /> เข้าดูตารางห้องเรียน</button>
      <button onClick={onBack} className="w-full text-gray-500 font-bold py-3 hover:text-gray-800 dark:hover:text-gray-200 hover:bg-gray-50 rounded-2xl transition-colors">ย้อนกลับ</button>
    </div>
  </div>
);
