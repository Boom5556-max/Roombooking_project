            ...
            <aside className="w-full lg:w-80 flex flex-col gap-6">
              ...
              <div className="bg-[#302782] text-white p-6 rounded-[32px] shadow-xl">
                <div className="flex items-center gap-2 mb-6 opacity-80">
                  <PieChart size={20} />
                  <span className="font-bold uppercase tracking-wider text-xs">ภาพรวมสถานะ</span>
                </div>
                <div className="space-y-4">
                  ...
                  <div className="h-2 bg-white/10 rounded-full overflow-hidden">
                    <div className="h-full bg-[#B2BB1E]" style={{ width: `${(stats.ready / stats.total) * 100}%` }}></div>
                  </div>
                  <div className="grid grid-cols-2 gap-4 mt-4">
                    <div className="bg-white/5 p-3 rounded-2xl">
                      <p className="text-[10px] uppercase opacity-50 font-bold">พร้อมใช้</p>
                      <p className="text-lg font-bold text-[#B2BB1E]">{stats.ready}</p>
                    </div>
                    <div className="bg-white/5 p-3 rounded-2xl">
                      <p className="text-[10px] uppercase opacity-50 font-bold">กำลังปรับปรุง</p>
                      <p className="text-lg font-bold text-red-400">{stats.maintenance}</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="flex-1">
              <div className="space-y-10">
                {groupedAndSortedRooms.length > 0 ? (
                  groupedAndSortedRooms.map(({ building, rooms: buildingRooms }) => (
                    <section key={building} className="relative">
                      <div className="flex items-center gap-4 mb-6 sticky top-0 bg-[#F8F9FE]/80 dark:bg-gray-900/80 backdrop-blur-md py-2 z-10">
                        ...
                        <div>
                          <h2 className="text-xl font-black text-[#302782] dark:text-white">{building}</h2>
                          <p className="text-sm text-black dark:text-white font-medium">{buildingRooms.length} รายการห้องเรียน</p>
                        </div>
                      </div>
                  ...
                  <div className="bg-white dark:bg-gray-800 rounded-[40px] p-20 text-center border-2 border-dashed border-gray-100">
                    <LayoutGrid size={64} className="mx-auto text-black dark:text-white mb-4" />
                    <p className="text-black dark:text-white font-bold">ไม่พบข้อมูลห้องเรียนในระบบ</p>
                  </div>
                )}
              </div>
            </div>
